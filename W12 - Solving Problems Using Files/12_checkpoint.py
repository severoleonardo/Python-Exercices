...
# This will keep track of the person with the yougest age
youngest_name = ""

# Go through each person in the list
for person_line in people:

    parts = person_line.split() # by default this will split on the spapce

    # Set variables for the two different parts
    name = parts[0]
    age = int(parts[1])

    # Check to see if this current person is younger than the yougest that we have seen so far 
    if age < youngest_age:
        ...
        # Save their name as the yougest name
        youngest_name = name
